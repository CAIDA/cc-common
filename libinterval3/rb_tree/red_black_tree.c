...
/*  RotationCallback is called whenever the tree is rotated and for each */
/*  affected by the rotation. To allow augmented trees algorithms. */
...
rb_red_blk_tree* RBTreeCreate( int (*CompFunc) (const void*,const void*),
			      ...
			      void (*PrintInfo)(void*),
            void (*RotationCallback)(rb_red_blk_node*)) {
  ...
  newTree->RotationCallback= RotationCallback;
...
/*            accordingly. Also calls the rotation callback function on each */
...
void LeftRotate(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  // Rotation callback for augmented trees
  tree->RotationCallback(x);
  tree->RotationCallback(y);
}
